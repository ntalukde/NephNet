...
def f1(output, target):
    with torch.no_grad():
        ...
        assert pred.shape[0] == len(target)
        f1_c = 0
        f1_c = skm.f1_score(target.detach().numpy(), pred, average = 'micro')
    return f1_c
...
def recall(output, target):
    with torch.no_grad():
        ...
        assert pred.shape[0] == len(target)
        f1_c = 0
        f1_c = skm.recall_score(target.detach().numpy(), pred, average = 'micro')
    return f1_c
...
def precision(output, target):
    with torch.no_grad():
        ...
        assert pred.shape[0] == len(target)
        f1_c = 0
        f1_c = skm.precision_score(target.detach().numpy(), pred, average = 'micro')
    return f1_c
