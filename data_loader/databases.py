...
class hdf5dataset(Dataset):
    ...
    def __init__(self, h5_path, shape = (7,32,32), training = True, transforms=None):
        ...
        #self.weight = torch.FloatTensor([1.0000, 0.0000, 1.5931, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000, 0.0000])
...
class hdf5dataset_test2(Dataset):
    '''
    Reads an HDF5 file with the following keys: train_data, train_labels, test_data, test_labels, test_ids. Train_data is a vector which is reshped to fit the shape variable. Works for both 2D and 3D input
    '''
    def __init__(self, h5_path, shape = (7,32,32), testing= True, transforms=None):
        st = pd.HDFStore(h5_path)
        self.store = st
        if testing:
            self.data = st['test1_data'].values
            self.label = st['test1_labels'].values.squeeze()
        else:
            self.data = st['test2_data'].values
            self.label = st['test2_labels'].values.squeeze()
        self.transforms = transforms
    ...
    def getIds(self):
        ...
        #NOTE: IDs should NOT be shuffled in order to keep ground truth label with the correct ID
        if self.testing:    
            ids = self.store['test1_ids'].values
        else:
            ids = self.store['test2_ids'].values
        return ids
