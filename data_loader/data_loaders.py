...
class hdf5_3d_dataloader_test2(BaseDataLoader):
    '''
    3D augmentations use a random state to peform augmentation, which is organized as a List rather than torch.Compose()
    '''
    def __init__(self, hdf5_path, batch_size, shuffle=True, shape = [7,32,32], validation_split=0.0, num_workers=1, testing = True, mean = None, stdev = None):
        rs = np.random.RandomState()
        if mean is None or stdev is None:
            mean = 0
            std = 1
        ...
        self.shape = shape
        importlib.reload(databases) #used to get load any recent changes from database class
        self.dataset = databases.hdf5dataset_test2(hdf5_path, shape = self.shape, testing = testing, transforms=trsfm)
        super(hdf5_3d_dataloader_test2, self).__init__(self.dataset, batch_size, shuffle, validation_split, num_workers)
...
#Not tested
class hdf5_3d_dataloader_sep_val(BaseDataLoader):
    '''
    3D augmentations use a random state to peform augmentation, which is organized as a List rather than torch.Compose()
    '''
    def __init__(self, hdf5_path, batch_size, shuffle=True, shape = [7,32,32], validation_split=0.0, num_workers=1, training=True, testing=True, validation=True, mean = None, stdev = None):
        rs = np.random.RandomState()
        if mean is None or stdev is None:
            mean = 0
            std = 1
        ...
        trsfm_val =  [t3d.Normalize(mean, stdev), 
                      t3d.ToTensor(True)]
